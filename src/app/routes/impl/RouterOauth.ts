...
class RouterOauth extends Routes {
	...
	private config(): void {
		...
		this.router.post('/config', (_req: Request, res: Response): void => {
			res.status(202).json({
				message: 'config',
				sucess: true,
			});
	...
	private login(): void {
		...
		this.router.get('/login', (_req: Request, res: Response): void => {
...
export default new RouterOauth().router;